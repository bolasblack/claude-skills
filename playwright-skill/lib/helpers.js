...
async function launchBrowser(browserType = 'chromium', options = {}) {
  const defaultOptions = {
    ...
    channel: 'chrome'  // Use system Chrome by default
  };
...
/**
 * Detect running dev servers on common ports (concurrent)
...
async function detectDevServers(customPorts = []) {
  ...
  const checkPort = (port) => new Promise((resolve) => {
    const req = http.request({
      ...
      if (res.statusCode < 500) {
        resolve({ port, found: true });
      } else {
        resolve({ port, found: false });
      }
    });
  ...
  const detectedServers = results
    .filter(r => r.found)
    .map(r => {
      console.log(`  Found server on port ${r.port}`);
